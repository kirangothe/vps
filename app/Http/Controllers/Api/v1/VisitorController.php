...
class VisitorController extends Controller
{
    ...
    public function changeStatus($id, $status, $remarks)
    {
        $visitor         = VisitingDetails::findOrFail($id);
        $visitor->status = $status;
        $visitor->rejection_remarks = $remarks;
        // $visitor->checkin_at = date('y-m-d H:i');
        $visitor->save();
        try {
            // $visitor->visitor->notify(new VisitorConfirmation($visitor));
            return $this->successResponse(['status' => 200, 'message' => 'Visitor Status Updated Successfully.']);
        } catch (\Exception $e) {
